...
impl Il2CppType {
    fn read_pe<'pe, 'data>(
        ...
        let data = match ty {
            ...
            Il2CppTypeEnum::Array => match array_type_map.get(&raw_data) {
                Some(idx) => TypeData::ArrayType(*idx),
                None => {
                    let idx = array_types.len();
                    array_types.push(Il2CppArrayType::read_pe(reader, raw_data, type_map)?);
...
impl Il2CppGenericClass {
    fn read_pe<'pe, 'data>(
        ...
        type_map: &HashMap<u64, usize>,
    ) -> Result<Il2CppGenericClass> {
        let mut cur = reader.make_cur(vaddr)?;

        let type_ptr = cur.read_u64::<LittleEndian>()?;
        let type_index = type_map[&type_ptr];

        let context = Il2CppGenericContext::read_pe(&mut cur, generic_inst_map)?;
...
impl Il2CppMetadataRegistration {
    fn read_pe<'data>(
        ...
        for (i, addr) in generic_class_addrs.into_iter().enumerate() {
            generic_classes.push(Il2CppGenericClass::read_pe(
                &reader,
                addr,
                &generic_inst_map,
                &type_map,
        ...
        for addr in generic_inst_addrs {
            generic_insts.push(Il2CppGenericInst::read_pe(&reader, addr, &type_map)?);
        }
